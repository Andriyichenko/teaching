...
\vspace{3em}
(i) \quad $t > \abb{\textsf{A}}$ のとき，$f(x)$ は $x = \abb{\textsf{A}}$ で極大，$x = t$ で極小となる。また，$f(4) = \ab{\textsf{B}}$ であるから\\[0.6em]
\qquad\quad\  $f\left(\abb{\textsf{A}}\right) > 6$ となる $t$ の値の範囲を求めればよい。\\

\qquad したがって、$t$の範囲は$t > \dfrac{\ab{\textsf{CD}}}{\ab{\textsf{E}}}$でなる。

\vspace{3em}
(ii) \quad $t = \abb{\textsf{A}}$ のとき，区間 $x \leq 4$ における $f(x)$ の最大値は $f\left(\ab{\textsf{F}}\right) = \ab{\textsf{G}}$ となり，条件は満たされない。
